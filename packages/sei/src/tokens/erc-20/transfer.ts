...
import { erc20Abi, isAddress, type Account, type Address } from "viem";
import { formatSei } from "../../utils/format";
import { getTokenDecimals } from "../../utils/get-balance";
  ...
  try {
    ...
    if (!ticker) {
      ...
      if (!transactionReceipt || transactionReceipt.status === "reverted") {
        const errorMsg = `Transaction failed: ${JSON.stringify(
          transactionReceipt
        )}`;
        throw new Error(errorMsg);
      }
    ...
    const token_address = await agent.getTokenAddressFromTicker(
      ticker.toUpperCase()
    );
    if (!token_address) {
    ...
    if (decimals === null || decimals === undefined) {
      throw new Error(
        `Failed to retrieve token decimals for contract: ${token_address}`
      );
    }
