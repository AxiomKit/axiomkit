...
export class SupabaseKVProvider implements KeyValueProvider {
  ...
  private setupOptions: Required<NonNullable<SupabaseKVProviderConfig['setupOptions']>>;
  ...
  private async setupDatabase(): Promise<void> {
    try {
      ...
      // Then create the table using the function
      ...
      console.log(`✅ Database setup complete for table: ${this.tableName}`);
    } catch (error) {
      console.error(`❌ Failed to setup database for ${this.tableName}:`, error);
      throw error;
    }
  }
  ...
  private async createExecuteSqlFunction(): Promise<void> {
    try {
      // Try to create the execute_sql function using direct SQL
      const { error } = await this.client.rpc("execute_sql", {
        query: `
      ...
      if (error && error.message.includes("function execute_sql")) {
        // If execute_sql doesn't exist, we need to create it using a different approach
        // For now, we'll use the existing initializeTable method as fallback
        console.log("⚠️  execute_sql function not available, using fallback setup");
        await this.initializeTable();
      }
    } catch (error) {
      console.log("⚠️  Could not create execute_sql function, using fallback setup");
      await this.initializeTable();
    }
  }
}
