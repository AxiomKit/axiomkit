...
export class RequestTracker {
    ...
    try {
      ...
      if (!this.isStorageHealthy()) {
        if (this.shouldLog("warn")) {
          this.logger!.warn(
            "RequestTracker",
            "Storage is unhealthy, continuing without persisting request",
            { requestId: request.id }
          );
        }
      } else {
  ...
  async completeRequest(
    ...
    try {
      ...
      const errorMessage = `Failed to store completed request ${requestId} after retries: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
      if (this.shouldLog("error")) {
        this.logger!.error("RequestTracker", errorMessage, {
  ...
  private async storeWithRetry<T>(
    ...
    throw (
      lastError ||
      new Error(
        `Storage operation ${operationName} failed after ${maxRetries} attempts`
      )
    );
  }
  ...
  private isStorageHealthy(): boolean {
    const totalFailures = Array.from(this.storageFailures.values()).reduce(
      (sum, count) => sum + count,
      0
    );
  ...
  private cleanupStaleEntries(): void {
    ...
    for (const [id, context] of this.activeContexts.entries()) {
      if (
        !this.activeAgentRuns.has(context.agentRunId) ||
    ...
    const totalActive =
      ...
      this.activeAgentRuns.size +
      this.activeContexts.size +
      this.activeActionCalls.size;
  ...
  async cleanup(): Promise<void> {
    ...
    const actionCallPromises = Array.from(this.activeActionCalls.keys()).map(
      (id) =>
