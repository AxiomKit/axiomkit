...
export class InMemoryRequestTrackingStorage implements RequestTrackingStorage {
  ...
  async storeRequest(request: RequestTracking): Promise<void> {
    try {
      ...
      this.log("debug", `Stored request: ${request.id}`, {
        requestId: request.id,
        source: request.source,
      });
    } catch (error) {
      const errorMessage = `Failed to store request: ${
  ...
  async getRequest(requestId: string): Promise<RequestTracking | null> {
    try {
      ...
      this.log("debug", `Retrieved request: ${requestId}`, {
        requestId,
        found: !!result,
      });
      return result;
    } catch (error) {
      const errorMessage = `Failed to get request: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
  ...
  async storeModelCall(modelCall: ModelCall): Promise<void> {
    try {
      ...
      const errorMessage = `Failed to store model call: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
    ...
    try {
      ...
      if (criteria.timeRange) {
        if (!criteria.timeRange.start || !criteria.timeRange.end) {
          throw new Error(
            "Invalid criteria: timeRange must have start and end"
          );
        }
        if (
          typeof criteria.timeRange.start !== "number" ||
          typeof criteria.timeRange.end !== "number"
        ) {
          throw new Error(
            "Invalid criteria: timeRange start and end must be numbers"
          );
        }
        if (criteria.timeRange.start > criteria.timeRange.end) {
          throw new Error(
            "Invalid criteria: timeRange start must be before end"
          );
        }
        results = results.filter(
          (r) =>
      ...
      if (criteria.offset !== undefined) {
        if (typeof criteria.offset !== "number" || criteria.offset < 0) {
          throw new Error(
            "Invalid criteria: offset must be a non-negative number"
          );
        }
      ...
      if (criteria.limit !== undefined) {
        if (typeof criteria.limit !== "number" || criteria.limit < 0) {
          throw new Error(
            "Invalid criteria: limit must be a non-negative number"
          );
        }
      ...
      const errorMessage = `Failed to query requests: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
  ...
  async getMetrics(criteria: MetricsQueryCriteria): Promise<AggregatedMetrics> {
    try {
      ...
      this.log(
        "debug",
        `Metrics computed: ${totalRequests} requests, ${completedRequests.length} completed`
      );
      return result;
    } catch (error) {
      const errorMessage = `Failed to get metrics: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
      this.log("error", errorMessage, { criteria, error });
      throw new Error(errorMessage);
    ...
    try {
      ...
      for (const request of requests) {
        try {
          ...
          this.log("warn", "Skipping invalid request in breakdown", {
            requestId: request?.id,
            error,
          });
          continue;
      ...
      return Array.from(groups.entries()).map(([groupKey, groupRequests]) => {
        try {
          ...
          const completedRequests = groupRequests.filter(
            (r) => r.status === "completed"
          );
          ...
          const groupTokenUsage =
            groupTokenUsages.length > 0
          ...
          const groupAverageResponseTime =
          ...
          this.log("warn", "Invalid group data in breakdown", {
            groupKey,
            error,
          });
          return {
          ...
          if (request.agentRuns && Array.isArray(request.agentRuns)) {
            for (const agentRun of request.agentRuns) {
              if (
                agentRun &&
                agentRun.contexts &&
                Array.isArray(agentRun.contexts)
              ) {
              ...
              if (
                ...
                for (const context of agentRun.contexts) {
                  if (
                    context &&
                    context.actionCalls &&
                    Array.isArray(context.actionCalls)
                  ) {
                  ...
                  if (
                    ...
                    for (const action of context.actionCalls) {
                      if (
                        action &&
                        action.modelCalls &&
                        Array.isArray(action.modelCalls)
                      ) {
                        for (const modelCall of action.modelCalls) {
                          if (modelCall && modelCall.callType) {
                            const group: Record<string, string> = {
                              ...
                              provider:
                                modelCall.metrics?.provider || "unknown",
                              callType: modelCall.callType,
                            };
                            keys.push(JSON.stringify(group));
                          }
  ...
  async getAllRequests(): Promise<RequestTracking[]> {
    try {
      ...
      const errorMessage = `Failed to get all requests: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
      this.log("error", errorMessage, { error });
      throw new Error(errorMessage);
  ...
  async getAllModelCalls(): Promise<ModelCall[]> {
    try {
      ...
      const errorMessage = `Failed to get all model calls: ${
        error instanceof Error ? error.message : "Unknown error"
      }`;
      this.log("error", errorMessage, { error });
      throw new Error(errorMessage);
...
export class NoOpRequestTrackingStorage implements RequestTrackingStorage {
  ...
  async queryRequests(
    _criteria: RequestQueryCriteria
  ): Promise<RequestTracking[]> {
    return [];
  }

  async getMetrics(
    _criteria: MetricsQueryCriteria
