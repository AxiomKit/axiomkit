...
import { cli, cliExtension } from "@axiomkit/cli";
...
const goalContexts = context({
  type: "goal",
  schema: z.object({
    id: string().describe("A unique ID for this specific goal"),
  }),
  ...
  render() {
    console.log("Render Exist In Context");
...
const genericMessageOutput = output({
  description: "Send a general message back to the user.",
  instructions: "Use plain text for your message.",
  schema: z.string(),
  handler(data) {
    console.log("Agent Says (Generic Message):", data);
    return { data };
  },
  examples: [`<output type="message">Hello there!</output>`],
});
  ...
  actions: [
    action({
      ...
      handler(data, ctx, _agent) {
        ...
        console.log("Add new Task", data, ctx);
