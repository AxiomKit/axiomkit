...
* **Source**: An external system - such as Discord, Telegram, CLI, or a custom API - delivers incoming data to the agent. These systems are integrated via **extensions**.
* **Listener**: An **input** handler, defined either within the agent or an extension, continuously listens for specific external events (e.g., a new user message).
