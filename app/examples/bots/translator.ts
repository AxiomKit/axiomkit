...
- Languages used: ${
      Array.from(translationStats.languagesUsed).join(", ") || "None yet"
    }
...
💡 Recent translations: ${
      state.memory.translationHistory
        .slice(-3)
        .map((t) => `${t.from} → ${t.to}: "${t.original.substring(0, 30)}..."`)
        .join("\n") || "None"
    }
    ...
    .map(
      (conv) =>
        `User: "${conv.userInput.substring(
          0,
          50
        )}..." → Bot: "${conv.botResponse.substring(0, 50)}..."`
    )
...
translatorContext.setActions([
  action({
    ...
    schema: z.object({
      ...
      includePronunciation: z
        .boolean()
        .optional()
        .describe("Include pronunciation guide"),
      includeCulturalNotes: z
        .boolean()
        .optional()
        .describe("Include cultural context notes"),
  ...
  action({
    ...
    schema: z.object({
      preferredLanguages: z
        .array(z.string())
        .optional()
        .describe("User's preferred languages"),
      formalStyle: z
        .boolean()
        .optional()
        .describe("Prefer formal translation style"),
      preserveFormatting: z
        .boolean()
        .optional()
        .describe("Preserve original formatting"),
      includePronunciation: z
        .boolean()
        .optional()
        .describe("Include pronunciation guides"),
      culturalNotes: z
        .boolean()
        .optional()
        .describe("Include cultural context notes"),
  ...
  action({
    ...
    schema: z.object({
      limit: z
        .number()
        .optional()
        .describe("Number of recent translations to show"),
      languagePair: z
        .string()
        .optional()
        .describe("Filter by language pair (e.g., 'en-es')"),
    }),
    handler: async (args, ctx) => {
  ...
  action({
    ...
    schema: z.object({
      context: z
        .string()
        .optional()
        .describe("Context for suggestions (e.g., 'business', 'travel')"),
    }),
    handler: async (args, ctx) => {
      const { context } = args;
...
async function performAdvancedTranslation({
  ...
  if (includeCulturalNotes) {
    const notes: Record<string, string> = {
      ...
      bonjour:
        "In France, 'bonjour' is typically used until around 6 PM, then 'bonsoir' is used.",
      こんにちは:
        "In Japan, bowing slightly while saying this greeting shows respect.",
      你好: "In Chinese culture, this greeting is often accompanied by a slight nod.",
      안녕하세요:
        "In Korean culture, this is a polite greeting used with people you don't know well.",
      "xin chào":
        "In Vietnamese culture, this is a friendly greeting used throughout the day.",
    };
...
function generateLanguageSuggestions(memory: any, context?: string) {
  ...
  if (context === "business") {
    return commonPairs.filter((pair) =>
      ["en", "es", "fr", "de"].includes(pair.to)
    );
  }

  if (context === "travel") {
    return commonPairs.filter((pair) =>
      ["es", "fr", "it", "pt"].includes(pair.to)
    );
  }
...
async function analyzeTranslation(args: any) {
  ...
  return {
    analysis: {
      ...
      culturalContext:
        "This translation considers cultural nuances and context.",
